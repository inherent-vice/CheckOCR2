...
- `python scripts\source_gui_smoke.py --entrypoint "python check_capture_ocr.py" --isolated-appdata --require-ready --require-settings-file --min-window-width 1000 --min-window-height 600`: smoke-test a source launcher with fast OCR readiness, isolated settings, and minimum window size.
- `python -m PyInstaller build_app.spec`: build the OneDIR Windows package under `dist/CheckCaptureOCR_V6.1/`.
- `python scripts\package_smoke.py dist\CheckCaptureOCR_V6.1\CheckCaptureOCR_V6.1.exe --timeout 45 --require-package-metadata --require-ocr-ready --require-settings-file --isolated-appdata --min-window-width 1000 --min-window-height 600`: smoke-test the packaged EXE, metadata, GUI Ready state, clean settings-file load, and minimum window size.
