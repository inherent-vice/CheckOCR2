...
class CheckCaptureOCRApp(tk.Tk):
    ...
    def open_output_folder(self):
        ...
        try:
            ...
            self.logger.info(f"출력 폴더 열기 시도 - 시스템: {system}, 경로: {cleaned_path}")

            if system == "Windows":
                # Windows에서는 os.startfile이 더 안정적입니다.
                # os.startfile은 슬래시와 백슬래시 모두 잘 처리하지만, 일관성을 위해 백슬래시로 통일
                cleaned_path_windows = cleaned_path.replace('/', '\\')
                self.logger.info(f"Windows 형식 경로 변환 후: {cleaned_path_windows}")

                # 폴더 존재 여부 확인 (로컬 경로만 확인, UNC는 os.startfile에 맡김)
                # UNC 경로는 os.path.exists 확인이 불안정하며, os.startfile에 맡깁니다.
                ...
                if not is_unc:
                    if messagebox.askyesno("폴더 생성", f"폴더가 존재하지 않습니다.\n생성하시겠습니까?\n\n경로: {cleaned_path_windows}"):
                        ...
                        self.logger.info(f"출력 폴더 생성됨: {cleaned_path_windows}")
                    else:
                        return

                # os.startfile로 폴더 열기 시도
                os.startfile(cleaned_path_windows)
                self.logger.info("출력 폴더 열기 (Windows os.startfile) 시도 완료")

                # UNC 경로인 경우 추가 안내 (사용자에게 네트워크 상태 확인 유도)
                if is_unc:
                     # messagebox.showinfo("UNC 경로 안내", "네트워크 드라이브 또는 UNC 경로일 수 있습니다.\n네트워크 연결 상태 및 접근 권한을 확인해보세요.")
                    pass
            elif system == "Darwin": # macOS
                # macOS는 open 명령어로 파일/폴더/URL을 엽니다.
                # UNC 경로는 smb:// 형식으로 변환 시도
