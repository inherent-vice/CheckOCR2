...
- Moved the Tk application shell into `checkocr2/app.py`; the root
  `check_capture_ocr.py` file is now a compatibility launcher/import alias for
  existing entrypoints and tests.
...
- Moved the legacy `OCRWorkflowManager` class into
  ...
  manager module.
- Removed the workflow manager's direct Tk `messagebox` import; the app shell
  now injects export-dialog callbacks when constructing `OCRWorkflowManager`.
- Moved legacy workflow run assembly into `checkocr2/workflow_execution.py`,
  leaving `OCRWorkflowManager` to own the EasyOCR reader, tracking state, and
  compatibility wrappers while the helper constructs adapters and finalizes the
  success run report.
...
- Added `scripts/check_ocr_evidence_bundle.py` to fail closed when OCR evidence
  ...
  Dry-run matrix reports are rejected with a concise matrix-level error instead
  of expanding every dry-run candidate into repeated failures.
...
- Added worker-thread exception callbacks so unexpected OCR worker failures are
  logged and routed through the stopped queue path instead of failing silently
...
- Added `scripts/check_ocr_repeatability.py` and evidence-bundle
  `--repeatability-json` / `--require-repeatability` support so candidate
  promotion can require three matching non-dry-run benchmark reports.
...
- Normalized Excel/table blank values so imported empty cells and in-memory
  NaN values are rendered, copied, and exported as empty strings instead of the
  literal `"nan"` text.
...
- `python -m pytest --basetemp $env:TEMP\checkocr2-pytest`: 470 passed after the repeatability evidence-bundle gate changes.
...
- `python -m pytest tests\test_check_ocr_repeatability_script.py tests\test_check_ocr_evidence_bundle_script.py --basetemp $env:TEMP\checkocr2-repeatability-bundle3`: 18 passed for three-run OCR benchmark repeatability checks, fixture consistency, and final evidence-bundle repeatability requirements.
...
- `python -m pytest tests\test_excel_table_modules.py tests\test_data_manager.py --basetemp $env:TEMP\checkocr2-excel-nan`: 17 passed for Excel/table NaN-to-blank handling across import, grid copy/render, export row preparation, and data-manager compatibility.
- `python -m pytest tests\test_worker.py tests\test_ocr_actions.py --basetemp $env:TEMP\checkocr2-worker-exception`: 12 passed for daemon worker exception callbacks and OCR worker failure log/stopped queue handling.
- `.\.analysis_tmp\package_venv\Scripts\python.exe -m PyInstaller build_app.spec --noconfirm --clean`: clean release venv package build passed after the workflow-execution helper and evidence-bundle reporting slice; global `python -m PyInstaller build_app.spec --noconfirm` still fails by design on this machine because global Python contains GUI/contrib OpenCV packages.
- `python scripts\package_smoke.py dist\CheckCaptureOCR_V6.1\CheckCaptureOCR_V6.1.exe --timeout 45 --require-package-metadata --require-ocr-ready --require-settings-file --isolated-appdata --ocr-ready-mode real --ocr-ready-timeout 180 --max-package-size-mb 650 --max-startup-seconds 5 --min-window-width 1000 --min-window-height 600 --require-clean-exit`: strict real package smoke passed on the fresh build with package size `596.409 MB`, startup `1.125` seconds, window size `1044x788`, clean exit code `0`, build date `2026-05-12T10:50:04+00:00`, isolated settings file, and no forbidden OpenCV dist-info.
- `python scripts\source_gui_smoke.py --entrypoint "python check_capture_ocr.py" --isolated-appdata --require-ready --require-settings-file --min-window-width 1000 --min-window-height 600 --require-clean-exit`: source GUI fast-OCR smoke passed after the workflow-execution helper and evidence-bundle reporting slice with startup `1.016` seconds, window size `1044x788`, `runtime_state="Ready"`, `ocr_ready=true`, clean GUI exit code `0`, isolated settings-file verification, and cleanup.
- `python scripts\source_gui_smoke.py --entrypoint "python Check_Capture_Excel_V6.1_배포.py" --isolated-appdata --require-ready --require-settings-file --timeout 45 --ocr-ready-timeout 45 --min-window-width 1000 --min-window-height 600 --require-clean-exit`: compatibility launcher smoke passed after OCR workflow manager module extraction with startup `1.015` seconds, window size `1044x788`, `runtime_state="Ready"`, `ocr_ready=true`, clean GUI exit code `0`, isolated settings-file verification, and cleanup.
- `python scripts\source_gui_smoke.py --entrypoint "python -m checkocr2.main" --isolated-appdata --require-ready --require-settings-file --timeout 45 --ocr-ready-timeout 45 --min-window-width 1000 --min-window-height 600 --require-clean-exit`: package bootstrap source smoke passed after OCR workflow manager module extraction with startup `1.016` seconds, window size `1044x788`, `runtime_state="Ready"`, `ocr_ready=true`, clean GUI exit code `0`, isolated settings-file verification, and cleanup.
...
- `python -m pytest tests\test_logging_and_main.py tests\test_async_ocr_initialization.py tests\test_completion_actions.py tests\test_coordinate_actions.py tests\test_folder_actions.py tests\test_grid_actions.py tests\test_grid_edit_actions.py tests\test_grid_refresh_actions.py tests\test_grid_update_actions.py tests\test_keyboard_actions.py tests\test_lifecycle_actions.py tests\test_ocr_actions.py tests\test_options_actions.py tests\test_presets.py tests\test_runtime_status_actions.py tests\test_section_frame.py tests\test_settings_actions.py tests\test_window_actions.py --basetemp $env:TEMP\checkocr2-app-shell-extract`: 135 passed after moving `CheckCaptureOCRApp` into `checkocr2/app.py` and reducing `check_capture_ocr.py` to the compatibility launcher.
- Full package gate after the workflow-execution helper and evidence-bundle reporting slice passed with `ruff`, 437-test `pytest`, `compileall`, benchmark dry-runs, matrix dry-run, canonical source launcher reaching `Ready`, clean PyInstaller build, and strict real package smoke at `596.409 MB`, `1.125` seconds startup, build date `2026-05-12T10:50:04+00:00`, `1044x788` window size, isolated settings-file verification, and clean GUI exit.
- `python -m pytest tests\test_ocr_workflow_manager.py tests\test_completion_actions.py tests\test_async_ocr_initialization.py --basetemp $env:TEMP\checkocr2-manager-dialogs`: 51 passed after removing the direct Tk messagebox import from `checkocr2/ocr_workflow_manager.py` and injecting export dialog functions from the app shell.
- `python -m pytest tests\test_workflow_execution.py tests\test_ocr_workflow_manager.py tests\test_check_ocr_evidence_bundle_script.py --basetemp $env:TEMP\checkocr2-workflow-exec-evidence2`: 38 passed after moving legacy workflow run assembly into `checkocr2/workflow_execution.py` and making dry-run matrix evidence-bundle failures concise.
...
- `python scripts\source_gui_smoke.py --entrypoint "python Check_Capture_Excel_V6.1_배포.py" --isolated-appdata --require-ready --require-settings-file --timeout 45 --ocr-ready-timeout 45 --min-window-width 1000 --min-window-height 600 --require-clean-exit`: passed with `window_title="📊 Check Capture OCR V6.1"`, `runtime_state="Ready"`, `ocr_ready=true`, window size `1044x788`, clean GUI exit code `0`, isolated settings-file verification, and cleanup.
- `python scripts\source_gui_smoke.py --entrypoint "python -m checkocr2.main" --isolated-appdata --require-ready --require-settings-file --timeout 45 --ocr-ready-timeout 45 --min-window-width 1000 --min-window-height 600 --require-clean-exit`: passed with `window_title="📊 Check Capture OCR V6.1"`, `runtime_state="Ready"`, `ocr_ready=true`, window size `1044x788`, clean GUI exit code `0`, isolated settings-file verification, and cleanup.
