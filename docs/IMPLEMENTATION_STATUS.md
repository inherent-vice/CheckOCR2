...
- `.\.analysis_tmp\package_venv\Scripts\python.exe -m PyInstaller build_app.spec --noconfirm --clean`: clean release venv package build passed after the workflow-manager export-dialog injection; global `python -m PyInstaller build_app.spec --noconfirm` still fails by design on this machine because global Python contains GUI/contrib OpenCV packages.
- `python scripts\package_smoke.py dist\CheckCaptureOCR_V6.1\CheckCaptureOCR_V6.1.exe --timeout 45 --require-package-metadata --require-ocr-ready --require-settings-file --isolated-appdata --ocr-ready-mode real --ocr-ready-timeout 180 --max-package-size-mb 650 --max-startup-seconds 5 --min-window-width 1000 --min-window-height 600 --require-clean-exit`: strict real package smoke passed on the fresh build with package size `596.408 MB`, startup `3.704` seconds, window size `1044x788`, clean exit code `0`, build date `2026-05-12T10:21:36+00:00`, isolated settings file, and no forbidden OpenCV dist-info.
- `python scripts\package_smoke.py dist\CheckCaptureOCR_V6.1\CheckCaptureOCR_V6.1.exe --timeout 90 --require-package-metadata --require-ocr-ready --require-settings-file --isolated-appdata --ocr-ready-mode real --ocr-ready-timeout 180 --max-package-size-mb 650 --max-startup-seconds 10 --min-window-width 1000 --min-window-height 600 --require-clean-exit`: follow-up package smoke rerun passed with startup `1.110` seconds on the same build.
- `python scripts\source_gui_smoke.py --entrypoint "python check_capture_ocr.py" --isolated-appdata --require-ready --require-settings-file --timeout 45 --ocr-ready-timeout 45 --min-window-width 1000 --min-window-height 600 --require-clean-exit`: source GUI fast-OCR smoke passed after workflow-manager export-dialog injection with startup `1.015` seconds, window size `1044x788`, `runtime_state="Ready"`, `ocr_ready=true`, clean GUI exit code `0`, isolated settings-file verification, and cleanup.
...
- Full app-shell package gate after the `checkocr2/app.py` move and workflow-manager dialog injection passed with `ruff`, 435-test `pytest`, `compileall`, benchmark dry-runs, matrix dry-run, all three source launchers reaching `Ready`, clean PyInstaller build, and strict real package smoke at `596.408 MB`, `3.704` seconds startup, build date `2026-05-12T10:21:36+00:00`, `1044x788` window size, isolated settings-file verification, and clean GUI exit.
...
- `python scripts\package_smoke.py dist\CheckCaptureOCR_V6.1\CheckCaptureOCR_V6.1.exe --timeout 45 --require-package-metadata --require-ocr-ready --require-settings-file --isolated-appdata --ocr-ready-mode real --ocr-ready-timeout 180 --max-package-size-mb 650 --max-startup-seconds 5 --min-window-width 1000 --min-window-height 600 --require-clean-exit`: real packaged EasyOCR initialization smoke passed with package size `596.408 MB`, startup time `3.704` seconds, window size `1044x788`, clean GUI exit code `0`, metadata build date `2026-05-12T10:21:36+00:00`, no forbidden OpenCV dist-info, isolated settings file under smoke `APPDATA`, temporary profile cleanup, and `Ready` state in the report.
