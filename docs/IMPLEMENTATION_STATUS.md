...
- `python -m pytest tests\test_log_actions.py tests\test_log_panel.py tests\test_logging_and_main.py tests\test_queue_dispatcher.py --basetemp $env:TEMP\checkocr2-log-core-parity`: 17 passed for GUI parity evidence on `INFO`, `WARNING`, `ERROR`, and `SUCCESS` log-widget insertion, Tk log-handler forwarding, queue dispatch, and log panel construction.
