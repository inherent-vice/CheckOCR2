...
- `python scripts\source_gui_smoke.py --entrypoint "python check_capture_ocr.py" --isolated-appdata --require-ready --require-settings-file --timeout 45 --ocr-ready-timeout 45 --min-window-width 1000 --min-window-height 600 --require-clean-exit`: source GUI fast-OCR smoke passed after OCR workflow manager module extraction with startup `1.016` seconds, window size `1044x788`, `runtime_state="Ready"`, `ocr_ready=true`, clean GUI exit code `0`, isolated settings-file verification, and cleanup.
...
- Full app-shell package gate after the `checkocr2/app.py` move and workflow-manager dialog injection passed with `ruff`, 435-test `pytest`, `compileall`, benchmark dry-runs, matrix dry-run, all three source launchers reaching `Ready`, clean PyInstaller build, and real package smoke at `596.408 MB`, `3.704` seconds startup, `1044x788` window size, isolated settings-file verification, and clean GUI exit.
