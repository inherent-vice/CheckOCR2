...
| Move legacy workflow manager out of root shell | `checkocr2/ocr_workflow_manager.py`, `tests/test_ocr_workflow_manager.py` | Manager class lives under `checkocr2`; focused and full tests passed after extraction. | Done |
...
| Keep package release gate green | Clean PyInstaller build plus `scripts/package_smoke.py ... --ocr-ready-mode real ... --require-clean-exit` | Latest recorded package smoke: `596.408 MB`, startup `1.188s`, build date `2026-05-12T09:56:37+00:00`, `Ready`, isolated settings, clean exit. | Done |
