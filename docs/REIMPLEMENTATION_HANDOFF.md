...
- Workflow, OCR, Excel, table, settings, paths, image-processing, runtime-state,
  run-report, queue-dispatch, and several UI-panel seams now have test coverage.
...
hidden-import cleanup, and real package smoke passed at about `596.35 MB` with
startup `1.141` seconds. After package-smoke real mode was added, the full test
suite was rerun and passed with 88 tests.
