    ...
    def execute_ocr_workflow_threaded(self, ui_settings, output_dir_str, save_detail_images_bool):
        ...
        try:
            ...
            for grid_index, row_data in enumerate(self.data_manager.excel_data):
                ...
                if not stock_code:
                    self.message_queue.put(("log", f"행 {grid_index+1}: 종목코드가 없어 건너<0xEB><0><0x8A><0xEB><0x9C><0x88>니다.", "WARNING"))
                ...
                try:
                    ...
                    if self.work_controller.skip_current:
                        self.message_queue.put(("log", f"종목 {stock_code}를 사용자 요청으로 건너<0xEB><0><0x8A><0xEB><0x9C><0x88>니다.", "INFO"))
...
class CheckCaptureOCRApp(tk.Tk):
    def __init__(self):
        ...
        self.geometry("1150x650") # 높이 약간 증가
        ...
        self.max_threads = tk.IntVar(value=4) # UI 표시용 (실제 스레드는 1개)
        ...
        try:
            while True:
                ...
                if msg_type == "progress":
                    ...
                    level_str, message = data[0], data[1]
    ...
    def _build_ui(self):
        ...
        main_container.grid_columnconfigure(0, weight=0, minsize=280) # 좌측 패널 너비 고정
    ...
    def _create_simple_toolbar(self):
        ...
        controls_frame = tk.Frame(toolbar)
        ...
        theme_lbl = tk.Label(controls_frame, text="테마:", font=('Segoe UI', 9))
        self.theme_manager.register_widget(theme_lbl, {'bg': 'primary', 'fg': 'white'})
        theme_lbl.pack(side='left', padx=(0, 3))
        
        self.theme_combo = ttk.Combobox(controls_frame, textvariable=self.theme_var, width=12, state="readonly", font=('Segoe UI', 8), style="TCombobox")
    ...
    def _create_left_panel_content(self, parent):
        ...
        # 마우스휠 바인딩 (scrollable_frame과 그 자식들에게)
        ...
        for widget in [canvas, scrollable_frame]: # 다른 위젯도 필요하면 추가
        ...
        self._create_options_section(scrollable_frame) # 고급 설정은 여기서 일부만
        self._create_preset_section(scrollable_frame) # 프리셋 섹션 추가
    ...
    def _create_right_panel_content(self, parent):
        ...
        log_controls = tk.Frame(log_section_frame)
        self.theme_manager.register_widget(log_controls, {'bg': 'white'})
        log_controls.pack(fill='x')

        clear_btn = tk.Button(log_controls, text="🗑️ 지우기", command=self.clear_log, font=('Segoe UI', 9), relief='flat', cursor='hand2', width=8)
        self.theme_manager.register_widget(clear_btn, {'bg': 'danger', 'fg': 'white', 'activebackground': 'dark'})
        clear_btn.pack(side='left', padx=(0,5))

        save_btn = tk.Button(log_controls, text="💾 저장", command=self.save_log, font=('Segoe UI', 9), relief='flat', cursor='hand2', width=8)
        self.theme_manager.register_widget(save_btn, {'bg': 'success', 'fg': 'white', 'activebackground': 'dark'})
    ...
    def _create_section_frame_styled(self, parent, title, fill_parent=False):
        ...
        section_container.pack(fill='x' if not fill_parent else 'both', expand=fill_parent, pady=(0, 8)) # 간격 조정

        title_frame = tk.Frame(section_container, height=24) # 높이 조정
        self.theme_manager.register_widget(title_frame, {'bg': 'primary'})
        title_frame.pack(fill='x')
        title_frame.pack_propagate(False)
        
        title_lbl = tk.Label(title_frame, text=title, font=('Segoe UI', 9, 'bold'))
        self.theme_manager.register_widget(title_lbl, {'bg': 'primary', 'fg': 'white'})
        title_lbl.pack(side='left', padx=8, pady=3) # 패딩 조정
    ...
    def _create_file_section(self, parent):
        ...
        excel_input_frame.pack(fill='x', pady=(2,0))
        
        self.excel_entry = tk.Entry(excel_input_frame, textvariable=self.input_excel_path, font=('Segoe UI', 9), relief='solid', bd=1)
        ...
        excel_browse_btn = tk.Button(excel_input_frame, text="찾기", command=self.browse_input_excel, font=('Segoe UI', 8), relief='flat', cursor='hand2', width=6)
        ...
        output_lbl = tk.Label(output_frame, text="출력 폴더:", font=('Segoe UI', 9, 'bold'))
        ...
        output_input_frame.pack(fill='x', pady=(2,0))
        self.output_entry = tk.Entry(output_input_frame, textvariable=self.output_folder_path, font=('Segoe UI', 9), relief='solid', bd=1)
        ...
        output_browse_btn = tk.Button(output_input_frame, text="찾기", command=self.browse_output_folder, font=('Segoe UI', 8), relief='flat', cursor='hand2', width=6)
    ...
    def _create_coordinates_section(self, parent):
        ...
        click_lbl = tk.Label(click_frame, text="클릭 포인트:", font=('Segoe UI', 10, 'bold'))
        self.theme_manager.register_widget(click_lbl, {'bg': 'white', 'fg': 'on_surface'})
        click_lbl.pack(side='left')
        click_btn = tk.Button(click_frame, text="위치지정", command=self.relocate_clickpoint, font=('Segoe UI', 9), relief='flat', cursor='hand2', width=10)
        ...
        for area_name, func, color_key in areas:
            ...
            area_lbl = tk.Label(area_frame, text=f"{area_name}:", font=('Segoe UI', 10, 'bold'))
            self.theme_manager.register_widget(area_lbl, {'bg': 'white', 'fg': 'on_surface'})
            area_lbl.pack(side='left')
            area_btn = tk.Button(area_frame, text="영역지정", command=func, font=('Segoe UI', 9), relief='flat', cursor='hand2', width=10)
        ...
        preview_all_btn = tk.Button(preview_frame, text="🔍 전체 영역 미리보기", command=self.show_area_preview, font=('Segoe UI', 9, 'bold'), relief='flat', cursor='hand2', width=28, pady=4)
    ...
    def _create_timing_section(self, parent):
        ...
        paste_lbl = tk.Label(left_timing, text="붙여넣기 딜레이(초):", font=('Segoe UI', 10, 'bold'))
        self.theme_manager.register_widget(paste_lbl, {'bg': 'white', 'fg': 'on_surface'})
        paste_lbl.pack(anchor='w')
        paste_entry = tk.Entry(left_timing, textvariable=self.paste_delay, font=('Segoe UI', 10), width=8, relief='solid', bd=1)
        self.theme_manager.register_widget(paste_entry, {'bg': 'white', 'fg': 'on_surface'})
        paste_entry.pack(fill='x', pady=(5,0))
        ...
        load_lbl = tk.Label(right_timing, text="로딩 딜레이(초):", font=('Segoe UI', 10, 'bold'))
        self.theme_manager.register_widget(load_lbl, {'bg': 'white', 'fg': 'on_surface'})
        load_lbl.pack(anchor='w')
        load_entry = tk.Entry(right_timing, textvariable=self.loading_delay, font=('Segoe UI', 10), width=8, relief='solid', bd=1)
        self.theme_manager.register_widget(load_entry, {'bg': 'white', 'fg': 'on_surface'})
        load_entry.pack(fill='x', pady=(5,0))
    ...
    def _create_options_section(self, parent):
        ...
        # 상세 이미지 저장 체크박스
        save_img_cb = tk.Checkbutton(section, text="상세 이미지 저장 (영역별 개별 파일)", variable=self.save_detail_images, font=('Segoe UI', 10))
        ...
        save_img_cb.pack(anchor='w', pady=(0,5))

        # 'KBP' 코드 건너뛰기 체크박스
        self.skip_kbp_var = tk.BooleanVar(value=self.settings_manager.get_advanced('skip_kbp_code', True)) # 기본값은 설정에서
        skip_kbp_cb = tk.Checkbutton(section, text="'KBP' 코드 건너뛰기 (빈 값으로 완료 처리)", variable=self.skip_kbp_var, font=('Segoe UI', 10), command=self.save_advanced_ui_to_settings) # 변경 시 바로 저장
        ...
        skip_kbp_cb.pack(anchor='w', pady=(0,5))

        # 고급 설정 UI (참고용 스레드 설정만 남음)
        adv_frame = tk.Frame(section)
        self.theme_manager.register_widget(adv_frame, {'bg': 'white'})
        adv_frame.pack(fill='x', pady=(5,0))
        
        thread_lbl = tk.Label(adv_frame, text="최대 스레드(참고용):", font=('Segoe UI', 10))
        self.theme_manager.register_widget(thread_lbl, {'bg': 'white', 'fg': 'on_surface'})
        thread_lbl.pack(side='left')
        thread_entry = tk.Entry(adv_frame, textvariable=self.max_threads, font=('Segoe UI', 10), width=3, relief='solid', bd=1, state='readonly') # 읽기 전용으로
        self.theme_manager.register_widget(thread_entry, {'bg': 'light', 'fg': 'on_surface'}) # 읽기 전용 배경색
        thread_entry.pack(side='left', padx=(3,2))

    def _create_preset_section(self, parent):
        section = self._create_section_frame_styled(parent, "💾 프리셋 관리")

        combo_frame = tk.Frame(section)
        self.theme_manager.register_widget(combo_frame, {'bg': 'white'})
        combo_frame.pack(fill='x', pady=(0,5))

        preset_lbl = tk.Label(combo_frame, text="프리셋 선택:", font=('Segoe UI', 9))
        self.theme_manager.register_widget(preset_lbl, {'bg': 'white', 'fg': 'on_surface'})
        preset_lbl.pack(side='left', padx=(0,5))

        self.preset_combo = ttk.Combobox(combo_frame, width=20, state="readonly", font=('Segoe UI', 9), style="TCombobox")
        self.preset_combo.pack(side='left', fill='x', expand=True)
        self.preset_combo.bind("<<ComboboxSelected>>", lambda e: self.apply_selected_preset())

        btn_frame = tk.Frame(section)
        self.theme_manager.register_widget(btn_frame, {'bg': 'white'})
        btn_frame.pack(fill='x', pady=(5,0))

        load_preset_btn = tk.Button(btn_frame, text="적용", command=self.apply_selected_preset, font=('Segoe UI', 8), relief='flat', width=6)
        self.theme_manager.register_widget(load_preset_btn, {'bg': 'success', 'fg': 'white', 'activebackground':'dark'})
        load_preset_btn.pack(side='left', padx=(0,3))

        delete_preset_btn = tk.Button(btn_frame, text="삭제", command=self.delete_selected_preset, font=('Segoe UI', 8), relief='flat', width=6)
        self.theme_manager.register_widget(delete_preset_btn, {'bg': 'danger', 'fg': 'white', 'activebackground':'dark'})
        delete_preset_btn.pack(side='left', padx=(0,10))
        
        self.preset_name_entry = tk.Entry(btn_frame, font=('Segoe UI', 9), relief='solid', bd=1, width=15)
        self.theme_manager.register_widget(self.preset_name_entry, {'bg':'white', 'fg':'on_surface'})
        self.preset_name_entry.insert(0, "새 프리셋 이름")
        self.preset_name_entry.pack(side='left', fill='x', expand=True, padx=(0,3))

        save_preset_btn = tk.Button(btn_frame, text="현재 설정 저장", command=self.save_current_as_preset, font=('Segoe UI', 8), relief='flat')
        self.theme_manager.register_widget(save_preset_btn, {'bg': 'primary', 'fg': 'white', 'activebackground':'dark'})
    ...
    def _create_center_excel_grid(self, parent):
        ...
        self.theme_manager.register_widget(clear_all_btn, {'bg': 'warning', 'fg': 'on_surface', 'activebackground':'dark'}) # 경고색, 텍스트 어둡게
        clear_all_btn.pack(side='right', padx=(5,0))


        tree_frame = tk.Frame(grid_section) # Treeview와 스크롤바를 담을 프레임
        ...
        self.grid_tree.bind('<Button-3>', self.show_context_menu_ui) # 우클릭 메뉴
        ...
        self.grid_tree.bind('<Control-v>', lambda e: self.paste_from_clipboard_ui()) # 그리드 포커스 시 Ctrl+V
        ...
        self.refresh_grid_tags() # 초기 태그 색상 적용
    ...
    def get_current_ui_settings(self):
        return {
            ...
            'advanced': self.settings_manager.data.get('advanced', self.settings_manager._get_default_advanced_settings()).copy()
        }
    ...
    def load_last_settings(self):
        try:
            ...
            self.update_advanced_ui_from_settings() # 고급 설정 UI 업데이트
            self.update_preset_combo() # 프리셋 콤보 업데이트
            self.theme_manager.change_theme(self.settings_manager.get_advanced('ui_theme', 'modern_blue')) # 저장된 테마 적용
        except Exception as e:
    ...
    def save_advanced_ui_to_settings(self):
        try:
            ...
            # self.settings_manager.set_advanced('ocr_confidence_threshold', round(self.confidence_threshold.get() / 100.0, 2)) # EasyOCR detail=0이라 미사용
            self.settings_manager.set_advanced('ocr_languages', ['en']) # 영어 고정
            self.settings_manager.set_advanced('thread_count', self.max_threads.get()) # 참고용
            self.settings_manager.save_settings()
            messagebox.showinfo("성공", "고급 설정이 저장되었습니다. (OCR 언어: 영어 고정)")
            self.logger.info("고급 설정 UI 값이 설정에 저장되었습니다. (영어 고정)")
            if self.ocr_workflow_manager.ocr_reader is None or \
               self.settings_manager.get_advanced('ocr_gpu_enabled') != self.use_gpu.get(): # GPU 설정 변경 시 재초기화
    ...
    def reset_advanced_settings_and_ui(self):
        if messagebox.askyesno("확인", "모든 고급 설정을 기본값으로 되돌리시겠습니까?"):
            ...
            self.ocr_workflow_manager.initialize_ocr() # OCR 엔진도 기본 설정으로 재초기화

    def update_advanced_ui_from_settings(self):
        try:
            self.use_gpu.set(self.settings_manager.get_advanced('ocr_gpu_enabled', False))
            # self.confidence_threshold.set(self.settings_manager.get_advanced('ocr_confidence_threshold', 0.2) * 100)
            self.max_threads.set(self.settings_manager.get_advanced('thread_count', 4))
            self.logger.debug("고급 설정 UI가 현재 설정값으로 업데이트되었습니다.")
        except Exception as e:
    ...
    def apply_selected_preset(self):
        ...
        if selected:
            ...
            if preset_settings:
                self.apply_settings_to_ui(preset_settings) # UI에 설정 적용
    ...
    def save_current_as_preset(self):
        ...
        current_settings = self.get_current_ui_settings() # UI에서 현재 설정 가져오기
    ...
    def clear_log(self):
        if self.log_text_widget:
            ...
            self.log_text_widget.config(state='disabled')
            self.logger.info("로그 화면이 지워졌습니다.")
    ...
    def save_log(self):
        ...
        if filename:
            try:
                ...
                self.logger.info(f"UI 로그 저장됨: {filename}")
            except Exception as e:
                messagebox.showerror("오류", f"로그 저장 실패: {e}")
                self.logger.error(f"UI 로그 저장 실패: {e}")
    ...
    def stop_processing_ui_initiated(self):
        if self.work_controller.is_running:
            ...
            self.message_queue.put(("log", "INFO", message)) # 로그에도 중단 메시지 기록
...
OCR 자동화 애플리케이션 (EasyOCR 기반)"""
    ...
    def run_ocr_process(self):
        ...
        self.progress_tracker.show() # 진행률 표시기 보이기
        ...
        self.worker_thread = threading.Thread(
            ...
            args=(current_ui_settings, output_dir, save_details), # 필요한 설정 전달
            daemon=True
        )
    ...
    def _validate_inputs_for_ocr(self):
        ...
        output_dir = self.output_folder_path.get().strip()
        if not self.data_manager.excel_data: # Excel 로드 여부 대신 데이터 존재 여부 체크
    ...
    def quit_app(self):
        if self.work_controller.is_running:
            if messagebox.askyesno("확인", "작업이 진행 중입니다. 정말로 종료하시겠습니까?", parent=self):
                self.work_controller.stop_work() # 진행 중인 작업 중단 요청
                if self.worker_thread and self.worker_thread.is_alive():
                    self.worker_thread.join(timeout=2) # 스레드 종료 대기 (최대 2초)
                self.destroy()
            else:
                return # 종료 취소
        else:
...
if __name__ == "__main__":
    ...
    app.protocol("WM_DELETE_WINDOW", app.quit_app) # 창 닫기 버튼 클릭 시 quit_app 호출
    app.mainloop()
